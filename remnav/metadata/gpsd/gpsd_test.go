...
`}
var jsTimeIsZero = []bool{true, true, false, false, true}
...
func TestUnmarshalProbe(t *testing.T) {
	for i, j := range js {
		...
		if probe.Time.IsZero() != jsTimeIsZero[i] {
			t.Fatalf("probe .Class %s, .Time %v (IsZero %v)\n",
				probe.Class, probe.Time,
				probe.Time.IsZero())
		}
		// fmt.Printf("%s\n", probe.Time.Format(RFC339NanoNatural))
	}
}
...
func TestUnmarshal(t *testing.T) {
	...
	for _, j := range js {
		...
		if probe.Class == "TPV" {
			...
			checkTime(t, "Time", "2022-10-20T20:32:53.001Z", tpv.Time)
