...
for capture in capture_list:
    ...
    while service_start_index < len(log_dic["service"]):
        ...
        try: tx_rx_bandwidth = tx_bytes * 8 / tx_rx_duration / 1000 
        except: tx_rx_bandwidth = 0
        try: rx_rx_bandwidth = tx_bytes * 8 / rx_rx_duration / 1000 
        except: tx_rx_bandwidth = 0
        try: sd_bandwidth = tx_bytes * 8 / service_duration / 1000 
        except: tx_rx_bandwidth = 0

        # requested bandwidth
        if service_start_line.service_transition_TS == 1686705460014:
            print ("debug")
        camera_start_index = bisect_left (dedup_a, service_start_line.service_transition_TS, key = lambda a: a.camera_TS)
        camera_stop_index = bisect_left (dedup_a, service_stop_line.service_transition_TS, key = lambda a: a.camera_TS)
        camera_bytes = 0
        index = camera_start_index
        while dedup_a[index].camera_TS < dedup_a[camera_stop_index].camera_TS or \
        ...
        while start_skip_index < len (log_dic["skip"]):
            ...
            FATAL ("Could not find line in skip log correspoiding to service start of channel {c} at {t}".format(
                c=service_start_line.channel, t=service_start_line.service_transition_TS))
            service_start_index += 1
            continue

        # dedup t2r just prior to the service start
        index = bisect_left (dedup_a, service_start_line.service_transition_TS, key = lambda a: a.tx_TS)
        ...
        # end of service
        fout.write ("stop_TS,{T}, stop_pkt,{p}, occ,{o}, est_t2r,{t1}, last_pkt_t2r,{t2}, t2r_bw,{b1:.2f}, r2r_bw,{b2:.2f}, sd_bw,{b3:.2f}, rq_bw,{b4:.2f},".format (
            ...
            t1=log_dic["service"][service_stop_index].est_t2r, t2=last_pkt_t2r, b1=tx_rx_bandwidth, b2=rx_rx_bandwidth, b3=sd_bandwidth, b4=rq_bandwidth))
