...
class VCState(GlobalThread):
    ...
    def update_state(self):
        ...
        pid.set_accel(self.acceleration)
...
class RemnavHijacker:
    ...
    def hijack(self, accel, CS):
        ...
        # Currently this is not being called....
...
class PIDController(GlobalThread):
    '''Interface to legacy PID controller'''
    def __init__(self, unit_test):
        self.socket = None
        self.accel = None
        self.unit_test = unit_test
        self.last_good_read_time = timestamp()
        self.request_tag = 0
        log_info("Create PID Controller")
    ...
    def runner(self):
        if self.unit_test:
            while running:
                time.sleep(.5)
            return
        log_info("PID Running")
        while running:
            if vc.state != STATE_REMOTE_DRIVER:
                if self.socket is not None:
                    log_info(f"Closing PID socket due to WAN State of {vc.wan_status}")
                    self.socket.close()
                self.socket = None
                self.accel = None
                time.sleep(.25)
                ...
                try:
                    ...
                    self.last_good_read_time = timestamp()
                    if result:
                        # log_info(f"Got PID Response {result.decode()}")
                        pass
                    else:
                        log_critical("PID Closed connection")
                        self.socket = None
                except BlockingIOError:
                    '''Socket read timed out. ignore'''
                    time.sleep(.25)
                except ConnectionError:
                    '''Probably the PID got shutdown'''
                    log_info("PID Connection error")
                    self.socket = None
                except ConnectionRefusedError:
                    if timestamp() - self.last_good_read_time > 10000:
                        log_info(f"Connection Refused to PID Controller for {(timestamp()-self.last_good_read_time)/1000} Secs")
                except:
                  log_info("Received Unknown Exception, ignoring")
                  time.sleep(.5)
                finally:
    ...
    def set_accel(self, accel):
        if self.socket is not None and accel != self.accel:
            #
            # Make message in format for PID controller
            #
            self.request_tag = self.request_tag + 1
            if accel > 0:
                gas, brake = accel, 0
...
if __name__ == "main":
    ...
    pid = PIDController('VC_UNIT_TEST' in os.environ)
